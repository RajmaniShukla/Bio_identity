...
function [recognized_img, ret_i]=facerecog(datapath,testimg)
...
for j = 1 : 10
    for i = 1 : 5
        str = strcat(datapath,'\',int2str(j),'\','f',' (',int2str(i),')','.bmp');
        img = imread(str);
        img = rgb2gray(img);
        [r, c] = size(img);
        temp = reshape(img',r*c,1);
          %% Reshaping 2D images into 1D image vector
...
recognized_img = strcat(int2str(recognized_index),'.bmp');